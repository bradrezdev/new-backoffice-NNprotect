...
from database.loyalty_points import LoyaltyPoints, LoyaltyTransactionType
...
class CompensationPlanV2:
    ...
    def process_fast_start_bonus(cls, session: sqlmodel.Session,
                                 order: Orders, period_id: int) -> Dict[str, any]:
        """
        Calcula y registra el Bono Rápido sobre primera compra.
        
        Porcentajes: 30%, 10%, 5% sobre VN en niveles 1, 2, 3
        """
    ...
    def process_loyalty_bonus(cls, session: sqlmodel.Session, order: Orders,
        ...
        """
        Procesa el Bono Lealtad: 25 puntos por recompra en primeros 7 días.
        
        Requisitos: PV >= 1,465, primeros 7 días, consecutivo
        """
    ...
    def update_cashback_tracking(cls, session: sqlmodel.Session, order: Orders,
                                user: Users, period_id: int) -> Dict[str, any]:
        """
        Actualiza el tracking de CashBack del usuario.
        
        Por cada 2,930 PV normal → derecho a 2,930 PV a PC
        """
        if hasattr(order, 'is_cashback') and order.is_cashback:
    ...
    def process_end_of_month(cls, session: sqlmodel.Session, period_id: int) -> Dict[str, any]:
        """
        Procesa todos los bonos de fin de mes.
        ...
        print("🔄 Reseteando volúmenes mensuales...")
        session.exec(
        ...
        print("✅ Procesamiento de fin de mes completado")
    ...
    def _reset_loyalty_points(cls, session: sqlmodel.Session, member_id: int):
        ...
        if user and hasattr(user, 'loyalty_points'):
            ...
            loyalty_tx = LoyaltyPoints(
                user_id=user.id,
                transaction_type=LoyaltyTransactionType.RESET,
                points=-old_balance,
